...
local ResetBank = function(bank)
    ...
    -- Scoreboard trigger
    TriggerEvent('qb-scoreboard:server:SetActivityBusy', Config.Banks[bank].type, false)
...
RegisterNetEvent('qb-bankrobbery:server:SetBankHacked', function(bank)
    ...
    TriggerEvent('qb-scoreboard:server:SetActivityBusy', Config.Banks[bank].type, true)
...
RegisterNetEvent('qb-bankrobbery:server:TrollyReward', function(type, bank, index)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local info = {worth = math.random(12000, 14000)} -- money bag worth

    local coords = GetEntityCoords(GetPlayerPed(src))
    if #(coords - Config.Banks[bank].trolly[index].coords.xyz) > 5 then
        exports['qb-core']:ExploitBan(src, "Bankrobbery-TrollyReward")
        return 
    end
...
RegisterNetEvent('qb-bankrobbery:server:LockerReward', function(type, bank, index)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    local coords = GetEntityCoords(GetPlayerPed(src))
    if #(coords - Config.Banks[bank].lockers[index].coords.xyz) > 5 then
        exports['qb-core']:ExploitBan(src, "Bankrobbery-LockerReward")
        return 
    end
...
RegisterNetEvent('qb-bankrobbery:server:RemoveGreenCard', function()
    ...
    Player.Functions.RemoveItem('security_card_01', 1, false)
...
RegisterNetEvent('qb-bankrobbery:server:RemoveGoldCard', function()
    ...
    Player.Functions.RemoveItem('security_card_02', 1, false)
