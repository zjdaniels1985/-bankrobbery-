RegisterNetEvent('qb-bankrobbery:client:UseGreenLaptop', function()
    if closestBank and Config.Banks[closestBank].type == 'fleeca' then
        local pos = GetEntityCoords(PlayerPedId())
        if #(pos - Config.Banks[closestBank].laptop.xyz) < 2.00 then
            local hasItem = QBCore.Functions.HasItem('laptop_green')
            if hasItem then
