local PacificZone = PolyZone:Create({
    vector2(227.14488220215, 213.12579345703), 
    vector2(266.51919555664, 199.21022033691), 
    vector2(287.84991455078, 261.22985839844), 
    vector2(250.8860168457, 275.25485229492)
 }, {
    name = "pacificbank",
    minZ = 100.00,
    maxZ = 105.50
...
local HitByLaser = function()
    ...
    QBCore.Functions.Notify("You see a laser hitting you..", "error", 2500)
end
...
LootStack = function(index) -- Globally used
    ...
    if Config.Banks['Vault'].stacks[index].model == `h4_prop_h4_cash_stack_01a` then
        ...
        TriggerServerEvent('qb-bankrobbery:server:DeleteObject', netId)
        while DoesEntityExist(sceneObject) do Wait(0) end
        ...
        ClearPedTasks(ped)
        -- Reward
        TriggerServerEvent('qb-bankrobbery:server:StackReward', 'money', index)
    elseif Config.Banks['Vault'].stacks[index].model == `h4_prop_h4_gold_stack_01a` then
        ...
        ClearPedTasks(ped)
        -- Reward
        TriggerServerEvent('qb-bankrobbery:server:StackReward', 'gold', index)
    end
...
RegisterNetEvent('qb-bankrobbery:client:UseGoldLaptop', function()
    if #(GetEntityCoords(PlayerPedId()) - vector3(257.43, 227.81, 101.68)) < 1.0 then
        local hasItem = QBCore.Functions.HasItem('laptop_gold')
        if hasItem then
...
RegisterNetEvent('qb-bankrobbery:client:UseGreyUsb', function()
    if not Config.Banks['Vault'].hacked then
        local hasItem = QBCore.Functions.HasItem('usb_grey')
        if hasItem then
            -- HACK ATTEMPT
